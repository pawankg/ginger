...
def _get_vgname(devNodePath):
    ...
    pvs = subprocess.Popen(["pvs", "--unbuffered", "--nameprefixes", "--noheadings",
...
def _get_partition_details(name):
    ...
    try:
        ...
        return {}
    dev['available'] = _is_available(name, dev['type'], dev['fstype'], dev['mountpoint'], majmin)
...
def _create_dasd_part(dev, size):
    ...
    devname = '/dev/' + dev
    p1_out = subprocess.Popen(["echo", "-e", "\'", p_str, "\'"], stdout=subprocess.PIPE)
    p2_out = subprocess.Popen(["fdasd", devname], stdin=p1_out.stdout, stderr=subprocess.PIPE, stdout=subprocess.PIPE)
    p1_out.stdout.close()
    out, err = p2_out.communicate()
    if p2_out.returncode != 0:
        wok_log.error("Creation of partition on DASD device %s failed", devname)
...
def _delete_dasd_part(dev, part_id):
    ...
    out, err = p2_out.communicate()
    if p2_out.returncode != 0:
        wok_log.error("Deletion of partition on DASD device %s failed", devname)
        raise OperationFailed("GINDASD0004E", {'err': err})
    return
